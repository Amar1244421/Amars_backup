...
namespace WebApplication5
{
    public class BusinessLayer
    {
        ...
        public void DeleteStudentRecord(Models.StudentInformation_Result studentcontactperson)
        {
            ...
            DB_ClassEntities db_dlt = new DB_ClassEntities();
            var students = db_dlt.Students.FirstOrDefault(s => s.StudentID == studentcontactperson.StudentID);
            if (students != null)
            {
                db_dlt.Students.Remove(students);
                db_dlt.SaveChanges();
            }
        }
        public void DeleteParentRecord(int id)
        {
            DB_ClassEntities db_dlt = new DB_ClassEntities();
            var students = db_dlt.Contact_Person.FirstOrDefault(s => s.Contact_PersonID == id);
            if (students != null)
            {
                db_dlt.Contact_Person.Remove(students);
                db_dlt.SaveChanges();
            }
        }
        }
